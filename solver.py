...
class LinearProgrammingProblem:
    ...
    def solve(self, method: str = None):
        ...
        method: "dantzig", "bland", "two-phase" hoặc None để tự chọn.
        """
        # 1) Chuẩn hóa về dạng standard form
        ...
        # 4) Biên dịch kết quả ra plain-text
        output = []
    ...
    def convert_to_standard_form(self):
        ...
        # 3) Xử lý dấu constraint
        for i in range(self.num_cons):
            sign = self.constraint_signs[i]
            if sign == 1:    # ≥
                self.constraint_matrix[i, :] = -self.constraint_matrix[i, :]
                self.constraint_rhs[i] = -self.constraint_rhs[i]
                self.constraint_signs[i] = -1  # chuyển thành ≤
            elif sign == 0:  # =
                self.constraint_signs[i] = -1   # chuyển hàng i thành ≤
